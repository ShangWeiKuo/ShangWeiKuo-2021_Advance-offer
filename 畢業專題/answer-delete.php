...
<?php
    ...
    if(isset($_GET['aid']) && isset($_GET['token']) && isset($_SESSION['account'])){
        $stmt = $conn->prepare('SELECT m_id FROM answer WHERE a_id = ?');
        $stmt->bind_param('d', $_GET['aid']);
        ...
        if(!$token_c || $r0 !== $_SESSION['m_id']){
            ...
            $stmt = $conn->prepare('UPDATE answer SET a_delete = 1 WHERE a_id = ?');
            $stmt->bind_param('d', $_GET['aid']);
            mysqli_stmt_execute($stmt);
            $stmt->close(); 
            if(isset($_GET['ref'])){
