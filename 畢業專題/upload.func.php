 ...
 */
function uploadFile($fileInfo, $allowExt = array('jpeg', 'jpg', 'gif', 'png', 'txt', 'pdf', 'docx', 'doc', 'ppt', 'pptx', 'zip', 'rar', '7z'), $maxSize = 16777216, $flag = true, $uploadPath = 'uploads') {
    // 存放錯誤訊息
    $res = array();
    ...
    if (!empty($res)){
        ...
        include("part/sql-connection.php");
        $stmt = $conn->prepare('INSERT INTO homework (m_id, c_id, h_file, h_name) VALUES (?, ?, ?, ?)');
        $stmt->bind_param('ddss', $_SESSION['m_id'], $_POST['cid'], $destination, $fileInfo['name']);
