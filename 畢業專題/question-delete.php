...
<?php
    ...
    if(isset($_GET['qid']) && isset($_GET['token']) && isset($_SESSION['account'])){
        $stmt = $conn->prepare('SELECT m_id FROM question WHERE q_id = ?');
        ...
        if(!$token_c || $r0 !== $_SESSION['m_id']){
            ...
            $stmt = $conn->prepare('UPDATE question SET q_delete = 1 WHERE q_id = ?');
            $stmt->bind_param('d', $_GET['qid']);
            mysqli_stmt_execute($stmt);
            $stmt->close(); 
            if(isset($_GET['ref'])){
